    ...
    <div class="page">
        <div ref="swpier-wrap" class='swpier-wrap swiper-container'>
            <div class="swiper-wrapper">
                <div class="swiper-slide" v-for="(item,index) in detailsGoods.images" :key="index"><img :src="item" alt="" /></div>
        ...
        <div class='goods-ele-main'>
            <div class='goods-title'>{{detailsGoods.title}}</div>
            <div class='price'>¥{{detailsGoods.price}}</div>
            <ul class='sales-wrap'>
                <li>快递：{{detailsGoods.freight}}元</li>
                <li>月销量{{detailsGoods.sales}}件</li>
            </ul>
        </div>
        <div class='reviews-main'>
            ...
            <div class="no-data" v-show="reviews<=0">没有相关评价</div>
            <div class='reviews-more' v-show="reviews.length>0" @click="$router.replace('/goods/details/review?gid='+gid)">查看更多评价</div>
        </div>
        ...
        <div ref="cart-panel" :class="panelShow?'cart-panel up':'cart-panel down'">
            <div ref="goods-info" class='goods-info'>
                ...
                <div ref="goods-img" class='goods-img'>
                    <img :src="detailsGoods.images && detailsGoods.images[0]" alt="" />
                </div>
            ...
            <div class='attr-wrap'>
                <div class='attr-list' v-for="(item,index) in attrs">
                    <div class='attr-name'>{{item.title}}</div>
                    <div class='val-wrap'>
                        <span :class="{val:true,active:item2.active}" v-for="(item2,index2) in item.values" @click="selectAttrs({index:index,index2:index2})">{{item2.value}}</span>
                    </div>
                </div>
                <!--<div class='attr-list'>-->
            ...
            <div class='amount-wrap'>
                ...
                <div class="amount-input-wrap">
                    ...
                    <div class='amount-input'><input type="tel" @input="checkAmount($event)" :value="amount" /></div>
...
<script>
    ...
    export default {
        ...
        computed:{
            ...mapState({
                detailsGoods:state=>state.goods.detailsGoods,
        ...
        created(){
            this.getDetails({gid:this.gid,success:()=>{
                this.$nextTick(()=>{
                    ...
                    document.title=this.detailsGoods.title;
                })
            }});
            this.getGoodsAttr({gid:this.gid});
        ...
        methods:{
            ...
            ...mapActions({
                ...
                getGoodsAttr:"goods/getGoodsAttr",
            ...
            submitData(){
                if(this.isMove){
                    ...
                    TweenMax.to(cloneImg, 1.5, {bezier:[{x:cloneImg.offsetLeft, y:-130},{x:cartIcon.offsetLeft, y:-cartIconTop}],onComplete:()=>{
                            //动画指定完成
                            cloneImg.remove();
                            this.isMove=true;
                            ...
                            let cartData={
                                gid:this.gid,
                                title:this.detailsGoods.title,
                                price:this.detailsGoods.price,
                                amount:this.amount,
                                img:this.detailsGoods.images[0],
                                checked:true,
                                freight:this.detailsGoods.freight,
                                attrs:attrs
                            }
