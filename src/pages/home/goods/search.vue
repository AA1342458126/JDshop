                ...
                <div class='attr-wrap'>
                    <div class='attr-title-wrap' @click="priceList.isShow=!priceList.isShow">
                        ...
                        <div class='price-wrap'>
                            <div class='price-input' @click.stop><input type="tel" placeholder="最低价" :value="minPrice" @keyup="setMinPrice({minPrice:$event.target.value})" /></div>
                            <div class='price-line'></div>
                            <div class='price-input' @click.stop><input type="tel" placeholder="最高价" :value="maxPrice" @keyup="setMaxPrice({maxPrice:$event.target.value})" /></div>
                        </div>
                ...
                <div>
                    <div class='attr-wrap' v-for="(item,index) in attrs" :key="index">
                        ...
                        <div class='item-wrap' v-show="item.isShow">
                            <div :class="{item:true,active:item2.checked}" v-for="(item2,index2) in item.param" :key="index2" @click="selectParam({index:index,index2:index2})">{{item2.title}}</div>
                        </div>
                    </div>
                    <div style='width:100%;height:1px;backgroundColor:#EFEFEF'></div>
                </div>
