    ...
    export default {
        ...
        methods:{
            ...
            addHistoryKwords(kwords){
                //接收传过来的热门搜索添加到历史记录
                let temKwords=kwords || this.kwords || "";
                if(this.historyKwordsArr.length>0 && temKwords){
                    for(let i=0;i<this.historyKwordsArr.length;i++){
                        //如果输入的关键字与历史记录相同，去重
                        if(this.historyKwordsArr[i]==temKwords){
                ...
                //排除空值
                if(temKwords){
                    this.historyKwordsArr.unshift(temKwords);
                }
